import type { ReadinessInputs, ReadinessResult } from "./types.js";
...
const blockerLabels: Partial<Record<keyof ReadinessInputs, string>> = {
...
const warningLabels: Partial<Record<keyof ReadinessInputs, string>> = {
...
export function readinessLevel(score: number): ReadinessResult["level"] {
...
export function calculateReadinessScore(inputs: ReadinessInputs): ReadinessResult {
  const breakdown = Object.fromEntries(
    Object.entries(weights).map(([key, weight]) => [
      key,
      inputs[key as keyof ReadinessInputs] ? weight : 0
    ])
  ) as ReadinessResult["breakdown"];

  const score = Object.values(breakdown).reduce((sum, value) => sum + value, 0);
  const blockers = Object.entries(blockerLabels)
    .filter(([key]) => !inputs[key as keyof ReadinessInputs])
    .map(([, label]) => label);
