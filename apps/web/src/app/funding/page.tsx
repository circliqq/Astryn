      ...
      <div className="space-y-5">
        <Panel className="flex flex-col gap-3 p-5 md:flex-row md:items-end">
          <label className="flex-1">
            <p className="mb-2 text-[11px] font-medium uppercase tracking-[0.04em]" style={{ color: "var(--text-3)" }}>Required ETH per wallet (mint price + gas buffer)</p>
            <Input
              className="w-40"
              type="number"
              step="any"
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_1fr]">
          <Panel className="p-5">
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="text-[13px] font-semibold" style={{ color: "var(--text-1)" }}>Smart Funding Plan</h2>
            </div>
            {isLoading ? (
              <p className="mt-4 text-[13px]" style={{ color: "var(--text-3)" }}>Loading wallets…</p>
              ...
              <div className="mt-4 space-y-2">
                {displayItems.map((item) => {
                  ...
                  return (
                    <div
                      key={item.walletId}
                      className="panel-section flex items-center justify-between px-4 py-3"
                    >
                      <div>
                        <p className="text-[13px] font-medium" style={{ color: "var(--text-1)" }}>{wallet?.name ?? item.walletId.slice(0, 8)}</p>
                        <p className="mt-0.5 text-[11px]" style={{ color: "var(--text-3)" }}>{item.reason}</p>
                      </div>
          ...
          <Panel className="p-5">
            <h2 className="text-[13px] font-semibold" style={{ color: "var(--text-1)" }}>Execution Guardrails</h2>
            <p className="mt-3 text-[13px] leading-6" style={{ color: "var(--text-3)" }}>
            ...
            <div className="panel-section mt-6 p-5">
              <p className="text-[11px] font-medium uppercase tracking-[0.04em]" style={{ color: "var(--text-3)" }}>Total required</p>
              <p className="mt-2 text-[28px] font-semibold tabular-nums leading-none" style={{ color: "var(--text-1)" }}>
                {totalEth} <span className="text-[14px] font-normal" style={{ color: "var(--text-2)" }}>ETH</span>
              </p>
            </div>
            <Button
